...
public class SocialMediaController {
	...
	public ResponseEntity<List<SocialMedia>> getSocialMedias(@RequestParam(value="name", required = false) String name) {
		...
		if(name == null){
			socialMedias = _socialMediaService.findAllCourses();
			if (socialMedias.isEmpty()) {
				return new ResponseEntity(HttpStatus.NO_CONTENT);
			}
			return new ResponseEntity<List<SocialMedia>>(socialMedias, HttpStatus.OK);
		}else{
			SocialMedia socialMedia = _socialMediaService.finByName(name);
			if(socialMedia == null){
	...
	public ResponseEntity<?> createSocialMedia(@RequestBody SocialMedia socialMedia,
		...
		if (_socialMediaService.finByName(socialMedia.getName()) != null) {
			return new ResponseEntity(HttpStatus.NO_CONTENT);
		}

		_socialMediaService.saveSocialMedia(socialMedia);
		SocialMedia socialMedia2 = _socialMediaService.finByName(socialMedia.getName());
